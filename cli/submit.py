...
Exemples (depuis la racine du repo) :
    python -m cli.submit                                   # constantes en dur
    python -m cli.submit --experiment lean-tune-optuna     # meilleure run de l'experiment
    python -m cli.submit --run-id 5191988f39ec443cb55a07ea80c28084
"""
import argparse
...
from training.model import DEFAULT_MAX_TRAIN, FEATURE_GROUPS, train_classifier
...
def backtest_sanity_check(cfg, feature_columns, ensemble_cfg, train_years) -> None:
    ...
    metrics = score(predict_proba_frame(clf, split.test, feature_columns))
    print(
...
def main() -> None:
    ...
    # Un seul passage de features (sur tout le dataset) : identiques pour le train et les fixtures.
    wide = build_eval_frame(cfg, res)

    # Entraînement : matchs joués des `train_years` dernières années, plafonnés (limite API).
    # Fixtures = matchs non joués. Les deux périmètres sont de simples filtres de lignes.
    cutoff = latest + timedelta(days=1)
    train_start = pl.select(pl.lit(cutoff).dt.offset_by(f"-{train_years}y")).item()
    train, future = select_rows(
        wide,
        train_pred=pl.col("outcome").is_not_null()
        & (pl.col("date") >= train_start)
        & (pl.col("date") < cutoff),
        test_pred=~pl.col("finished"),
    )
    train = train.sort("date").tail(DEFAULT_MAX_TRAIN)
    future = future.sort("date")
    ...
    # Fit final + prédiction des fixtures (probas valides + alignées via la couture).
    clf = build_model(ensemble_cfg, RANDOM_STATE)
    clf = train_classifier(train, clf, RANDOM_STATE, feature_columns)
    pred_frame = predict_proba_frame(clf, future, feature_columns)

    probs = pred_frame.select("p_away_win", "p_draw", "p_home_win").to_numpy()
